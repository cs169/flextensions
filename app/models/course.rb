class Course < ApplicationRecord
  ...
  def self.fetch_courses(token)
    make_canvas_request("#{ENV.fetch('CANVAS_URL')}/api/v1/courses", token, { 'include[]' => 'enrollment_type' })
  end
  ...
  def self.create_or_update_from_canvas(course_data, token, _user)
    ...
    course.create_default_form_setting unless course.form_setting
    course.create_default_course_settings unless course.course_settings
  ...
  def self.find_or_create_course(course_data, token)
    ...
    canvas_course = make_canvas_request("#{ENV.fetch('CANVAS_URL')}/api/v1/courses/#{canvas_id}", token)

    course = find_or_initialize_by(canvas_id: canvas_id)
    course.course_name = canvas_course['name'] || course_data['name']
    course.course_code = canvas_course['course_code'] || course_data['course_code']
    course.save!
    course
  end
  ...
  def self.sync_assignments(course_to_lms, token)
    ...
    assignments.each { |assignment_data| sync_assignment(course_to_lms, assignment_data) }
  ...
  def self.sync_assignment(course_to_lms, assignment_data)
    ...
    assignment.due_date = parse_canvas_date(assignment_data, 'due_at')
    assignment.late_due_date = parse_canvas_date(assignment_data, 'lock_at')
    assignment.save!
  end

  # Fetch users for a course from Canvas API
  def fetch_users_from_canvas(token, enrollment_type = nil)
    make_canvas_request("#{ENV.fetch('CANVAS_URL')}/api/v1/courses/#{canvas_id}/users", token, { 'enrollment_type' => enrollment_type })
  end
  ...
  def sync_users_from_canvas(token, role)
    ...
    UserToCourse.where(course_id: id, role: role).find_each do |user_to_course|
  ...
  # Helper methods
  def self.make_canvas_request(url, token, params = {})
    response = Faraday.get(url) do |req|
      req.headers['Authorization'] = "Bearer #{token}"
      req.headers['Content-Type'] = 'application/json'
      params.each { |key, value| req.params[key] = value if value.present? }
    end

    if response.success?
      JSON.parse(response.body)
    else
      Rails.logger.error "Failed to fetch data from Canvas: #{response.status} - #{response.body}"
      []
    end
  end

  def self.parse_canvas_date(data, key)
    if data['base_date'] && data['base_date'][key].present?
      DateTime.parse(data['base_date'][key])
    elsif data[key].present?
      DateTime.parse(data[key])
    end
  end

  def create_default_form_setting
    build_form_setting(
      documentation_desc: 'Please provide links to any additional details if relevant.',
